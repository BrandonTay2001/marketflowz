...
const Index = () => {
  ...
  const handlePurchase = async (workflow: Workflow) => {
    ...
    const contractAddress = '0x10156e02c7CC3ebDC87e94d6B2FAA14b20d7aa49';
    ...
    try {
      ...
      fetch("http://127.0.0.1:5000/workflow/purchase", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:5000/workflow/getTopWorkflows").then((response) => {
      if (response.ok) {
        response.json().then((data) => {
    ...
    fetch("http://127.0.0.1:5000/workflow/getAll").then((response) => {
      if (response.ok) {
        response.json().then((data) => {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.1:5000/dashboard/getPurchasedWorkflows?buyer=${address}`).then((response) => {
      if (response.ok) {
        response.json().then((data) => {
